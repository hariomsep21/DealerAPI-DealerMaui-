...
namespace DealerAPI.Controllers
{
    ...
    public class PaymentController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<PaymentDto>>> GetDuePayments()
        {
            try
            {
                ...
                var filteredDuePayments = duePayments
                    .Where(p => DateTime.Compare(p.DueDate.Date, currentDate.Date) < 0 &&
                                (p.StartDate - p.DueDate.Date).Days <= 60)
                    .ToList(); // Filter the payments locally

                var paymentDtos = filteredDuePayments.Select(p => new PaymentDto
                {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<IEnumerable<PaymentDto>>> GetUpcomingPayments()
        {
            try
            {
